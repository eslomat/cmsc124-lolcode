...
def error(expected):
    ...
    if expected == "HAI" and lexeme_to_match == "$":
        ...
        err = f"error: syntax error at line {line}, unterminated variable declaration section\n\n\tshould be:\n\t\t\tHAI\n\t\t        WAZZUP\n\t\t\t.\n\t\t\t.\n\t\t    ->  BUHBYE\n\t\t\t.\n\t\t\t.\n\t\t\tKTHXBYE\n"
    elif expected == "KTHXBYE" and lexeme_to_match == "$":
        err = f"error: syntax error at line {line}, unterminated main section\n\n\tshould be:\n\t\t        HAI\n\t\t\t.\n\t\t\t.\n\t\t    ->  KTHXBYE\n"
    elif expected == "line break" and lexeme_to_match == "$":
        err = f"error: syntax error at line {line}, expected a new line character or ',' after '{prev_lexeme}'\n\n\tshould be:\n\t\t\t{exhaustline(-1,-1)}\n\t\t     -> {exhaustline(1,0)}\n\n\tor should be:\n\t\t     -> {exhaustline(-1,-1)}, {exhaustline(1,0)}\n"
